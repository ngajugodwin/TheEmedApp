...
namespace eMed.View
{
    ...
    public partial class OutlookAppointmentForm : DevExpress.XtraBars.Ribbon.RibbonForm, IDXManagerPopupMenu
    {
        ...
        string _patient = String.Empty;
        public static OutlookAppointmentForm fromOutlookApptForm;
        #endregion
        ...
        {
            //do nothing
            if (appointment.CustomFields["Patient"] != null)
                _patient = appointment.CustomFields["Patient"].ToString();
            txtPatient.Text =  _patient;
        }
        string patientName;
        public virtual bool SaveFormData(Appointment appointment)
        {
            patientName = txtPatient.Text;
            appointment.CustomFields["Patient"] = patientName;
            return true;
        }
        private void AutoEmail()
        {
            string message = "Would you like to send an automated email?";
            ...
            if (result == DialogResult.Yes)
            {
                ...
                Email email = new Email(patientName);
                //email.btnSend.PerformClick();
                email.SendAutoEmail("Yes");
                
            }
            else
                MessageBox.Show("No probs");
        }
        public virtual bool IsAppointmentChanged(Appointment appointment)
        {
            if (_patient == appointment.CustomFields["Patient"].ToString())
                return false;
            else
                return true;
        }
        ...
        private void pictureBoxSearch_Click(object sender, EventArgs e)
        {
            ...
            var search = new Search(ButtonEventSearch.ApptSearch);
            search.ShowDialog();
        }
